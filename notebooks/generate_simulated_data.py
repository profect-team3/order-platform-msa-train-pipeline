...
def generate_store_profiles(num_stores):
    ...
    for i in range(num_stores):
        ...
        profiles[store_id] = {
            ...
            if CONFIG["num_days"] >= 60 and np.random.random() < 0.2
            else None,  # 20% 확률로 이벤트
        }
    return profiles
        ...
        while current_time < end_time:
            ...
            if current_time.hour == 0:
                ...
                for store_id, profile in store_profiles.items():
                    ...
                    daily_avg_rating = round(
                        profile["base_rating"] + rating_noise + day_count * 0.001, 2
                    )
            ...
            for store_id, profile in store_profiles.items():
                ...
                trend_multiplier = (
                    1 + (day_count * profile["trend"]) + np.random.normal(0, 0.005)
                )  # 트렌드 노이즈 추가
                weekday = current_time.weekday()
                # 평일별 다양화: 월(0) 낮음, 화수목 중간, 금(4) 높음, 주말 높음
                if weekday == 0:  # 월요일
                    weekday_multiplier = np.random.uniform(0.7, 0.9) * np.random.normal(
                        1.0, 0.1
                    )  # 노이즈 추가
                elif weekday == 4:  # 금요일
                    weekday_multiplier = np.random.uniform(1.3, 1.6) * np.random.normal(
                        1.0, 0.1
                    )
                elif weekday >= 5:  # 주말
                    weekday_multiplier = profile[
                        "weekend_multiplier"
                    ] * np.random.normal(1.0, 0.1)
                else:  # 화수목
                    weekday_multiplier = np.random.uniform(0.9, 1.1) * np.random.normal(
                        1.0, 0.1
                    )
                daily_multiplier = 1.0
                # 기본 시간대 multiplier (낮 시간 주문 다양화)
                if 6 <= current_time.hour <= 10:  # 아침
                    daily_multiplier = np.random.uniform(0.3, 0.7) * np.random.normal(
                        1.0, 0.2
                    )
                elif 14 <= current_time.hour <= 17:  # 오후
                    daily_multiplier = np.random.uniform(0.5, 1.0) * np.random.normal(
                        1.0, 0.2
                    )
                elif 3 <= current_time.hour <= 5:  # 새벽
                    daily_multiplier = np.random.uniform(0.1, 0.4) * np.random.normal(
                        1.0, 0.2
                    )
                for peak_name, (start, end, multiplier) in profile.get(
                    "peak_hours", {}
                ).items():
                    if peak_name == "late_night" and (
                        start <= current_time.hour or current_time.hour <= end
                    ):
                ...
                region_multiplier = CONFIG["region_multipliers"].get(
                    profile["region"], 1.0
                )  # 지역 multiplier
                ...
                if profile.get("event"):
                    ...
                    if (
                        event["start_day"]
                        <= day_count
                        <= event["start_day"] + event["duration"]
                    ):
                ...
                sales_amount = int(
                    order_count
                    * (profile["min_order_amount"] * np.random.normal(1.0, 0.1))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Filename for the generated CSV.",
    )
